...
class PowerFactor_Window:
  def __init__(self, master):
    ...
    self.buttonPwrFact = Tkinter.Button(self.frame,height=1, width=20,text=u"Set power factor",command=self.setPwrFactorbutton)
  ...
  def setPwrFactorbutton(self):
    global powerfactor, app
    powerfactor = self.PwrFactVariable.get()
    if (app != None):
      app.PowerFactorVariable.set(powerfactor)        
    self.master.destroy()
...
class Window(Frame):
  ...
  def init_window(self):
    ...
    subSetup.add_command(label='Custom Curve', command=lambda p=customName: self.settrainer(p))
  ...
  def Runoff(self):
    def run():
      global dev_trainer, runoff_loop_running, maxRollDownSpeed, maxRollDownSpeed, minRollDownSpeed, rollDownTargetTime
      self.RunoffButton.config(state="disabled")
...
##  ''').substitute(MaxSpeed = maxRollDownSpeed, RollDownTargetTime = '%3.1f'%rollDownTargetTime))
      ...
      while runoff_loop_running:#loop every 100ms
        ...
        if speed > maxRollDownSpeed:#speed above maxRollDownSpeed, start rolldown
          self.runoffVariable.set("STOP PEDALLING!")
          rolldown = True
        ...
        if speed <=maxRollDownSpeed-2 and rolldown:#rolldown timer starts when dips below maxRollDownSpeed
          ...
          self.runoffVariable.set("STOP PEDALLING! - Rolldown timer started %s " % ( round((time.time() - rolldown_time),1) ) )
          
        if speed < minRollDownSpeed and rolldown:#wheel stopped
          runoff_loop_running = False#break loop
          self.runoffVariable.set("Rolldown time = %s seconds (aim %3.1fs)" % (round((time.time() - rolldown_time),1), rollDownTargetTime))
    ...
    if self.RunoffButton.cget('text')=="2. Perform Runoff":#start runoff
      self.runoffVariable.set('Cycle to above %i kph then stop'%maxRollDownSpeed)
  ...
  def Start(self):
    ...
    def run():
      global dev_ant, dev_trainer, simulatetrainer, switch, power_curve, powerfactor
      if power_curve == "":
        if not headless: 
      ...
      try:
        while switch == True:
          ...
          calc_power=max(0, int((speed*factors[0] + factors[1])*float(powerfactor)))
